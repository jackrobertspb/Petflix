      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400">Organize your favorite pet videos</p>
          </div>
          <button
        ...
        {loading ? (
          <div className="text-center py-20">
            <div className="text-xl text-gray-400">Loading playlists...</div>
          </div>
        ) : playlists.length === 0 ? (
          <EmptyState
            icon="📂"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {playlists.map((playlist) => (
              <div
                key={playlist.id}
                className="bg-petflix-dark rounded-lg p-6 hover:bg-petflix-gray transition group"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <Link
                      to={`/playlist/${playlist.id}`}
                      className="text-xl font-bold text-white hover:text-petflix-orange transition group-hover:underline"
                    >
                      {playlist.name}
                    </Link>
                ...
                <Link
                  to={`/playlist/${playlist.id}`}
                  className="inline-block text-sm text-petflix-orange hover:underline"
                >
                  View Playlist →
                </Link>
              </div>
