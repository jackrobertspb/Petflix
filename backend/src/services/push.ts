...
export async function sendNotificationToUser(
  ...
  try {
    ...
    try {
      ...
      console.log(`🔔 [PUSH] Saving notification to database:`, {
        userId,
      ...
      console.log(`✅ Notification saved to database for in-app bell. ID: ${savedNotification?.id}`);
    } catch (dbError: any) {
      // If table doesn't exist, just log and continue (graceful degradation)
      if (dbError.code === 'PGRST205') {
        console.warn('⚠️ ⚠️ ⚠️ notifications table does not exist - in-app bell won\'t work!');
        console.warn('⚠️ Run migration: petflix/backend/src/db/add-notifications-table.sql');
      } else {
